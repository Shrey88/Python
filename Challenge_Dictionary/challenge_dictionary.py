...
# dictionaries holding the exits ( as they do t present). No change should
# be needed to the actual code
#
# Once that is working , create another dictionary that contains words that
# player may use. These words will be the keys, and their vlues will be
...
while True:
    ...
    print()
    if direction_key.get(direction) in exits.get(loc).keys():
        loc = (exits.get(loc)).get(direction_key.get(direction))
    else:
